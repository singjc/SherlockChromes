...
@click.command()
@click.option( '--in_chrom_csv', '--in_chromatograms_csv', help='Chromatogram csv file containing filename column containing peptide sequence. This file should be derived from osw_parser.' )
@click.option( '--in_truth', '--ground_truth_file', help='Single column ground truth file containing peptide sequences that you know are present. i.e. synthetic peptides.' )
@click.option( '--out_file', '--out_file', default="ground_truth_based_binary_labels.npy", help='Numpy file to write binary labels to.' )
def main( in_chromatograms_csv, ground_truth_file, out_file ):
